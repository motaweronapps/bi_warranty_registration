odoo.define('ye_dynamic_odoo.ReportEdit', function (require) {
"use strict";

    var core = require('web.core');
    var Base = require('ye_dynamic_odoo.BaseEdit');
    var ReportContent = require('ye_dynamic_odoo.ReportContent');
    var ReportProperty = require('ye_dynamic_odoo.ReportProperty');
    ...
    var ReportEdit = Base.EditBase.extend({
        ...
        onSaveSubTotal: function (nodeXpath, data, position) {
            ...
            newNode = $(newNode.cloneNode(true));
            const {tax, total, subtotal} = data, nodeFac = (label, field) => {
                let nodeNode = this.stringToXml(`<tr><td><strong>${label}</strong></td><td><span t-field="${field}" /></td></tr>`);
                newNode.find("t_body".replace("_", "")).append(nodeNode);
            };
            if (tax) {
                nodeFac("Taxes", tax.columnField.join("."));
            }
            if (total) {
                nodeFac("Total", total.columnField.join("."));
            }
            if (subtotal) {
                nodeFac("Subtotal", subtotal.columnField.join("."));
            }
            position == "append" ? nodeXpath.appendChild(newNode[0]) : nodeXpath.parentNode.insertBefore(newNode[0], nodeXpath);
            this.saveTemplate();
        },
                ...
                if (type == "component" && viewId) {
                    ...
                    this.getPdfTemplate(viewId).then(() => {
                        if (sortType == "field_column") {
                            ...
                            self.newTable(elXpath, viewId, position);
                        }else if (["sub_total"].includes(sortType)) {
                            self.newSubTotal(elXpath, viewId, position);
                        }
                        // el.remove();
                    });
                }
