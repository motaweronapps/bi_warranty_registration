...
"use strict";
        ...
        prepareFields: function () {
            ...
            if (dataRoot) {
                Object.keys(dataRoot).map((fName) => {
                    var field = {name: fName, string: fName}, fieldType = dataRoot[fName], strField = fieldType, model = fieldType.model;
                    if (model) {
                        field.relation = model;
                        field.type = "many2one";
                        strField = model.split(/[._]/).map((f) => this.capitalizeFirstLetter(f)).join(" ");
                    }
    ...
    var ReportPropertyWidget = Widget.extend({
        ...
        onChangeWidget: function (widgetName) {
            const {onChange} = this.props;
            onChange({name: "widget", value: widgetName});
        },
        ...
        renderWidget: function () {
            ...
            const {fieldNode} = data, {widget} = fieldNode.widgetOptions,
                propsSelect = {data: selectData, label: "Widget", value: widget || data.widget || false, onChange: this.onChangeWidget.bind(this)};
    ...
    return {ViewMoreView: ViewMoreView, SelectFieldModel: SelectFieldModel, Relation: Relation, ColorLine: ColorLine, Groups: Groups, WidgetOption: WidgetOption, CBCondition: CBCondition, Condition: Condition,
        ...
        GroupCheckBox: GroupCheckBox, Input: Input, Tab: Tab, Button: Button, TextArea: TextArea, ParentElNode: ParentElNode, WidgetBase: WidgetBase}
});
